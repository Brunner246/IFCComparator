...
class TestFuzzyHashmap(unittest.TestCase):
    ...
    def test_fuzzy_hashmap_equality(self):
        ...
        fmap2 = FuzzyHashmap(self.data2, self.tolerance, self.collector)
        self.assertTrue(fmap1, fmap2)

    def test_fuzzy_hashmap_inequality(self):
        fmap1 = FuzzyHashmap(self.data1, self.tolerance, self.collector)
        fmap3 = FuzzyHashmap(self.data3, self.tolerance, self.collector)
    ...
    def test_fuzzy_hashmap_nested_structure(self):
        ...
        fmap1 = FuzzyHashmap(nested_data1, self.tolerance, self.collector)
        fmap2 = FuzzyHashmap(nested_data2, self.tolerance, self.collector)
